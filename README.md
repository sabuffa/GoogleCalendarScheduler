...
Otherwise will return error string with error enclosed in xml tags 'error'
...
- errors will always be returned enclosed in error tags.

- each call into the api will only return a single error at a time, the first error it encounters

- each request must have one and only one name tag.  The name is the id of the calendar that is being operated on.  It should always be of the form *@group.calendar.google.com.  If a given name is not matched to an id in google calendars the system will give an error
...
- if request type is allocate, or free there can only be one date tag.  The time is required.
...
- allocation must have only one date tag and it is required

- allocation must have only one duration tag and it is required.  It is in a unit of minutes

- duration tag can only be on an allocate request

- allocation requests optionally can have a single description tag.  The contents of this tag will be put in the description field of the calendar event

- description can only be on an allocate request

- allocation requests optionally can have a single location tag.  The contents of this tag will be put in the title and location fields of the calendar event
